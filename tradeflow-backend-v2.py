...
def create_devis_pdf(data: dict, numero: str) -> str:
    """
    Génère un PDF devis conforme aux standards légaux français.
    Design professionnel avec logo, couleurs, signature.
    """
    ...
    c.rect(0, height - 88*mm, width, 3*mm, fill=1, stroke=0)

    # Logo + Nom entreprise
    c.setFillColor(ORANGE)
    c.setFont("Helvetica-Bold", 20)
    c.drawString(20*mm, height - 25*mm, "TradeFlow AI")

    c.setFillColor(WHITE)
    c.setFont("Helvetica", 10)
    c.drawString(20*mm, height - 33*mm, data.get("artisan_name", ARTISAN_NAME))
    c.drawString(20*mm, height - 39*mm, f"SIRET: {ARTISAN_SIRET}")
    c.drawString(20*mm, height - 45*mm, f"TVA: {ARTISAN_TVA}")
    c.drawString(20*mm, height - 51*mm, ARTISAN_ADDRESS)
    if ARTISAN_EMAIL:
        c.drawString(20*mm, height - 57*mm, ARTISAN_EMAIL)
    if ARTISAN_PHONE:
        c.drawString(20*mm, height - 63*mm, ARTISAN_PHONE)
    ...
    c.setFillColor(WHITE)
    c.setFont("Helvetica", 10)
    c.drawRightString(width - 20*mm, height - 38*mm, f"N° {numero}")
    c.drawRightString(width - 20*mm, height - 45*mm,
                      f"Date: {datetime.date.today().strftime('%d/%m/%Y')}")
    c.drawRightString(width - 20*mm, height - 52*mm,
    ...
    if urgence:
        ...
        c.drawRightString(width - 20*mm, height - 62*mm, "🔴 INTERVENTION URGENTE")

    # ─── BLOC CLIENT ────────────────────────────────────
    y = height - 105*mm

    c.setFillColor(LIGHT)
    c.rect(width/2, y + 4*mm, width/2 - 20*mm, -38*mm, fill=1, stroke=0)

    c.setFillColor(DARK)
    c.setFont("Helvetica-Bold", 9)
    c.drawString(width/2 + 5*mm, y, "ADRESSÉ À")
    c.setFont("Helvetica", 9)
    c.setFillColor(GRAY)
    c.drawString(width/2 + 5*mm, y - 7*mm, data.get("client_name", "Client"))
    if data.get("client_phone"):
        c.drawString(width/2 + 5*mm, y - 13*mm, data["client_phone"])
    if data.get("client_email"):
        c.drawString(width/2 + 5*mm, y - 19*mm, data["client_email"])
    if data.get("client_address"):
        addr = data["client_address"]
        c.drawString(width/2 + 5*mm, y - 25*mm, addr[:50])
    ...
    c.setFont("Helvetica", 9)
    c.setFillColor(GRAY)
    desc = data.get("description_travaux", "Travaux d'artisanat")
    c.drawString(40*mm, y, desc[:80])
    ...
    # En-tête tableau
    c.setFillColor(DARK)
    c.rect(20*mm, y, width - 40*mm, 9*mm, fill=1, stroke=0)
    c.setFillColor(WHITE)
    c.setFont("Helvetica-Bold", 9)
    c.drawString(23*mm,      y + 2.5*mm, "DÉSIGNATION")
    c.drawString(width - 98*mm, y + 2.5*mm, "QTÉ")
    c.drawString(width - 82*mm, y + 2.5*mm, "UNITÉ")
    c.drawString(width - 62*mm, y + 2.5*mm, "P.U. HT")
    c.drawString(width - 38*mm, y + 2.5*mm, "TOTAL HT")
    ...
    for i, item in enumerate(data["items"]):
        ...
        c.setFillColor(bg)
        c.rect(20*mm, y - 1.5*mm, width - 40*mm, 9*mm, fill=1, stroke=0)

        total_ligne = item["quantite"] * item["prix_unitaire_ht"]
        c.setFillColor(DARK)
        c.setFont("Helvetica", 9)

        desc_item = str(item["description"])[:52]
        c.drawString(23*mm,         y + 2*mm, desc_item)
        c.drawString(width - 98*mm, y + 2*mm, str(item["quantite"]))
        c.drawString(width - 82*mm, y + 2*mm, str(item.get("unite", "forfait")))
        c.drawString(width - 62*mm, y + 2*mm, f"{item['prix_unitaire_ht']:.2f} €")
        c.drawString(width - 38*mm, y + 2*mm, f"{total_ligne:.2f} €")
        y -= 9*mm
    ...
    box_h = 32*mm
    c.setFillColor(LIGHT)
    c.rect(width - 90*mm, y - box_h, 70*mm, box_h, fill=1, stroke=0)

    c.setFillColor(DARK)
    c.setFont("Helvetica", 9)
    c.drawString(width - 87*mm, y - 7*mm,  "Total HT:")
    c.drawRightString(width - 22*mm, y - 7*mm,  f"{data['total_ht']:.2f} €")

    c.drawString(width - 87*mm, y - 15*mm, f"TVA {data['tva_rate']:.0f}%:")
    c.drawRightString(width - 22*mm, y - 15*mm, f"{data['tva_montant']:.2f} €")

    # Total TTC en surbrillance
    c.setFillColor(DARK)
    c.rect(width - 90*mm, y - box_h, 70*mm, 12*mm, fill=1, stroke=0)
    c.setFillColor(ORANGE)
    c.setFont("Helvetica-Bold", 12)
    c.drawString(width - 87*mm, y - 27*mm,  "TOTAL TTC:")
    c.drawRightString(width - 22*mm, y - 27*mm, f"{data['total_ttc']:.2f} €")
    ...
    if data.get("notes"):
        ...
        c.setFillColor(GRAY)
        notes = str(data["notes"])[:120]
        c.drawString(20*mm, y - 6*mm, notes)

    # ─── SIGNATURE ──────────────────────────────────────
    sig_y = 38*mm
    c.setFillColor(LIGHT)
    c.rect(width - 88*mm, sig_y, 68*mm, 28*mm, fill=1, stroke=0)
    c.setFillColor(GRAY)
    c.setFont("Helvetica-Bold", 8)
    c.drawString(width - 85*mm, sig_y + 22*mm, "BON POUR ACCORD")
    c.setFont("Helvetica", 8)
    c.drawString(width - 85*mm, sig_y + 15*mm, "Signature du client:")
    c.drawString(width - 85*mm, sig_y + 5*mm,  f"Date: ___/___/______")
    ...
    c.drawCentredString(width/2, 9*mm, "TRADEFLOW AI")
    c.setFillColor(WHITE)
    c.setFont("Helvetica", 7)
    c.drawCentredString(width/2, 4*mm,
        f"Devis N°{numero} · Valable 30 jours · Document généré automatiquement")

    c.save()
    print(f"✅ PDF créé: {filename} ({data['total_ttc']}€ TTC)")
    return filename
...
async def full_pipeline(transcript: str, customer_number: str = "",
    ...
    # 4. Email au client si email disponible
    email_ok = await send_email_client(devis_data, numero, pdf_path, app_url)

    # 5. Sync Google Sheets via Make.com
